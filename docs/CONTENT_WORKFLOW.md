...
The kit's source is `tools/blender/build_world.py`. Each named asset is a collection; collections are hidden by default in the saved blend to keep the whole kit from overlapping. Unhide the collection you want to inspect. The saved blend is a reviewable source artifact, but the script is the regeneration source: direct blend edits are overwritten by a full build. To introduce hand-authored assets, add an explicit source-blend import/export step rather than relying on edits to generated collections.
...
`content/catalog.mjs` contains weapon, origin, enemy and boss definitions. `GameWorld.think` currently implements shared perception, pursuit, wind-up and attack logic, with Meep navigation fallback around cover. Add a small number of distinct moves before increasing variant count. The planned next step is Meep behavior trees with serialized gameplay state; do not introduce a parallel AI engine.
