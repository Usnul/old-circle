...
`tools/blender/build_lods.py` follows the world export and saves `old-circle-lods.blend` plus ignored `lods.json`. Its terrain uses an eight-metre interior grid and the exact two-metre boundary vertices of the detailed tiles. Large static models use Blender decimation while retaining material groups. The compiler writes model bounds, byte counts and distant-model mappings into manifest version 2. After changing terrain or a model with a distant representation, regenerate both levels; the stitching regression reads the shipped native geometry and verifies every perimeter. Inspect mixed-detail terrain, monument silhouettes and foliage in the browser after compilation.

`WorldStream` bounds scene changes and request concurrency, uses distance hysteresis, and keeps a previous representation visible until its replacement loads. The workshop reports scene counts and geometry residency for travel checks. Visited geometry uses a fixed catalogue cache because Meep's native BVH arena cannot currently reclaim departed model allocations (MEEP-012); do not repeatedly remove/re-register the same native geometry. Banner skins pause offstage and resume their original registered instance for the related native allocation limitation. Gameplay collision stays fully resident and always uses full authored geometry. Before play, serialized geometry is warmed into browser CacheStorage under the manifest's content fingerprint, so future regions can still load after a server disconnect. Storage denial falls back to a finite session byte cache. A first install downloads all geometry bytes; only nearby detail is decoded and placed into the scene.

The final build step bakes the collision-tested navigation surface into `packages/game/src/content/navigation.bin` using Meep's BinaryBuffer. Run `node tools/world-check.mjs bake` after changing terrain, roads or solid placement; the bake refuses blocked progression routes. Simulation loads this shared surface and prepares Meep NavMesh tiles for enemy homes, retaining at most 32 in a least-recently-used cache. Pathfinding never reruns the terrain/clearance raster during a tick. Queries outside a tile fail explicitly; new roaming systems need a tile corridor or a higher-level route instead of an unbounded whole-world search.
...
The three backdrop mountain meshes use warped summit profiles, eroded saddles and smooth normals. Nine overlapping placements conceal the northern edge, with lower central ridges leaving sky behind the final halo. Their buried skirts extend below the terrain and they remain outside playable collision. Check the opening view and both northern approaches when editing their height or spacing; visual continuity at the edge is separate from the navigation report.
...
After changing floors or collision, run the asset build and `node tools/world-check.mjs bake`. This validates the native topology, every outdoor road and the entrance-to-relic paths and writes both navigation assets. Dungeon regression tests cover every room pair and a complete physical walk to each reward and return, plus enemy pursuit between stacked floors, personal collection and old-world migration. Movement time allowances follow route length; a long straight approach can take more than eight seconds. Clear scenery across the entrance ramp as well as the room footprints, and continue the road's terrain grade beneath it. Room names and the map come from the same definitions. Workshop composition views include approaches and rewards in every dungeon; inspect entrances, low ceilings, stairs and rewards under automatic exposure in daylight and darkness.
...
`content/charms.mjs` defines the five regional blessings and their burdens. Ownership derives from the matching relic; only one charm is equipped. Change it through the sequenced hearth command, and apply its effects in shared simulation rather than only in the UI. The equipment panel shows modified weapon damage and focus cost. Round fractional focus costs consistently before affordability checks so a displayed 9.6-focus spell accepts exactly 9.6 focus. Test ownership, costs, real damage, resource recovery and delayed command replay when adding another modifier.
...
Icons use local FLUX.1-dev, bfloat16, sequential CPU offload, 384 px, 16 steps and fixed seeds. The palette/prompt is shared across sword, bow, seal, flask and five exploration charms. Existing images and their provenance are preserved; use `--only charm-glass --force` to replace a chosen icon. The PyTorch ceiling adapts to existing GPU usage while retaining a 1 GiB buffer below the shared 18 GiB budget. Audio uses the local Stable Audio 3 Small SFX weights and bundled text conditioner in `H:\git\stable-audio-3-small-sfx`, eight steps, fixed seeds, half precision and chunked decode. Both output folders contain prompt/seed provenance. Audio generation preserves existing WAVs; remove only the individual output you intend to regenerate.
...
Sopra loads the generated WAVs through Meep's asset manager, with positional one-shots, attenuation, voice limits and ambient loops. The 24 `step-*` and `paw-*` samples are isolated single impacts for grass, gravel, stone, snow, wood and sand; their provenance includes the original duration and trim. The old two-impact `step.wav` is retained as source output and is not used for walking. Listen to regenerated output in context before approving it.

Run `node tools/bake-acoustics.mjs` after changing terrain, collision, room layouts or acoustic materials. The standard asset build includes it. `world/acoustics.mjs` assigns ground, masonry, timber and snow three-band absorption/transmission; `static-geometry.mjs` provides exactly the same transformed convex bodies as physics. The bake places probes at listener height on outdoor routes, a sparse world grid, cave sections and all dungeon storeys. Meep traces 256 multi-bounce rays per probe and serializes its native field. The shipped bake has 1,033 probes and no visibility/pathing graph. Keep acoustic pathfinding disabled for the open world.

Runtime sound uses at most 24 acoustic voices, eight rays per source at 20 Hz, and a visible-probe/shelter lookup at 5 Hz. Native convolution receives only the effects bus, returns through master gain and caps its tail at two seconds. Nearby braziers provide up to three positional fire loops; sheltered spaces reduce the dry wind bed. The workshop reports active sources, selected probe, shelter, tail and solve time. Test both sides of a doorway and stacked floors, and listen to the transitions: geometry and graph tests verify routing, not perceived sound quality.
...
Network changes must preserve the host-only rollback world, independent recipient baselines and the returning character allowlist. `interest.mjs` defines spatial and count budgets; nearby projectiles need their owner records. `record-codec.mjs` is wire schema 3: field and string ordinals are protocol data, so changing them requires a framing/protocol version update. Numbers remain exact through prediction and replay. Keep source-only AI paths out of recipient views and verify that a saturated view stays below the native fragment receiver limit. The server defaults to eight players based on the measured tick budget in `ARCHITECTURE.md`; raising capacity requires measuring packet processing as well as simulation.
