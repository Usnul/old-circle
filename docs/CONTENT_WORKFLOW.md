...
Run `node tools/bake-acoustics.mjs` after changing terrain, collision, room layouts or acoustic materials. The standard asset build includes it. `world/acoustics.mjs` assigns ground, masonry, timber and snow three-band absorption/transmission; `static-geometry.mjs` provides exactly the same transformed convex bodies as physics. The bake places probes at listener height on outdoor routes, a sparse world grid, cave sections and both dungeon storeys. Meep traces 256 multi-bounce rays per probe and serializes its native field. The shipped bake has 557 probes and no visibility/pathing graph. Keep acoustic pathfinding disabled for the open world.
