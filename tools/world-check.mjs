...
import {bakeDungeonNavigation,encodeDungeonNavigation,withDungeonNavigation} from '../packages/game/src/world/dungeon-navigation.mjs';
import {DUNGEONS,dungeonPoint} from '../packages/game/src/world/dungeons.mjs';
...
try{
  if(mode==='los')console.log(JSON.stringify(new SpatialAtlas(world).visibility(point(args[1]),point(args[2])),null,2));
    ...
    const atlas=new SpatialAtlas(world).build(),interiors=bakeDungeonNavigation(world);
    const navigation=withDungeonNavigation({tile:()=>atlas},new Map(interiors.map(a=>[a.id,a]))),pathBetween=(a,b)=>navigation.tile(a).path(a,b);
    const dungeonRoutes=interiors.map(interior=>{
      const d=DUNGEONS.find(d=>d.id===interior.id),start=point(d.id),end=dungeonPoint(d,d.treasure.at);end[2]+=1.5;
      const path=pathBetween(start,end);return {id:d.id,name:d.name,faces:interior.faceCount,reachable:path.reachable,length:path.length,reason:path.reason};
    });
    if(mode==='bake'){
      ...
      for(const route of dungeonRoutes){
        if(!route.reachable)throw new Error(`Dungeon route blocked: ${route.id}: ${route.reason}`);
        console.log(`${route.name}: ${route.faces} layered faces, ${route.length.toFixed(1)} m from entrance to relic.`);
      }
      await writeFile(resolve('packages/game/src/content/dungeon-navigation.bin'),encodeDungeonNavigation(interiors));
    }
    else if(mode==='path'){const result=pathBetween(point(args[1]),point(args[2]));console.log(JSON.stringify(result,null,2));if(!result.reachable)process.exitCode=1;}
    else if(mode==='report'){
      ...
      const result={schema:1,spacing:atlas.spacing,faces:atlas.faceCount,routes,dungeons:dungeonRoutes,views,heatmap:atlas.samples,notes:'Sampled outdoor navigation and separately authored dungeon layers. Visibility uses gameplay collision BVHs.'};
      ...
      if(dungeonRoutes.some(r=>!r.reachable))process.exitCode=1;
