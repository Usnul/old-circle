...
def build_dungeons(world,cube,mesh,finish,cone):
    for dungeon in world['dungeons']:
        ...
        def prism(x0,n0,x1,n1,y0,y1,thickness,material='limestone'):
        ...
        def solid_wall(a,b,ya,yb,h,thickness=.55,material='limestone'):
            ...
            for offset in [.08,h-.1]:
                ...
                trim=cube(p,(length+.03,thickness+.10,.12),'stoneLight',.02);trim.rotation_euler[2]=math.atan2(dn,dx)
        ...
        for f in floors:
            ...
            if f['roof']:prism(x0-.15,n0-.15,x1+.15,n1+.15,ya+4.5,yb+4.5,.22,'stoneDark')
        ...
        for wall in dungeon['partitions']:
            ...
            for v in [p,q]:cube(point(v[0],v[1],y+1.45),(.72,.72,2.9),'stoneLight',.035)
        # A small blind arcade above the threshold marks the dungeon from the cave.
        front=height(floors[-1],-3)
        for side in [-1,1]:
            for row in range(7):cube(point(side*2.35,-3,front+.3+row*.6),(.6,.85,.58),'stoneLight',.035)
        for i in range(11):
            ...
            mesh('Threshold arch',vertices,[(0,1,3,2),(4,6,7,5),(0,4,5,1),(2,3,7,6),(0,2,6,4),(1,5,7,3)],'stoneLight')
