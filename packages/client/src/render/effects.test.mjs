...
import {effect,AMBIENT_EFFECTS,FOOTSTEP_EFFECTS} from './effects.mjs';
...
test.each([['levitation',1.4],['motes',6],...Object.entries(AMBIENT_EFFECTS).map(([kind,p])=>[kind,p.life]),...Object.entries(FOOTSTEP_EFFECTS).map(([kind,p])=>[kind,p.life])])('%s fades its visible contribution before its GPU lifetime ends',(kind,life)=>{
