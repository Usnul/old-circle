...
export function worldMapMarkup(player){
  ...
  const places=LANDMARKS.filter(l=>l.kind!=='hearth').map(l=>{
    const [x,y]=worldToMap(l.position[0],l.position[2]),boss=Object.values(BOSSES).find(b=>b.landmark===l.id),done=boss&&player.seals.includes(boss.seal);
    return `<g class="map-place"><title>${l.name}${done?' · Seal recovered':''}</title><circle cx="${x}" cy="${y}" r="3" fill="${done?'#e6c984':'#233830'}" stroke="#e6c984" stroke-width=".7"/><text x="${x}" y="${y-7}" text-anchor="middle">${l.name}</text></g>`;
  }).join('');
  ...
  return `<div class="panel-top"><div><div class="eyebrow">The known lands</div><h2>All roads turn inward</h2></div><button class="close" aria-label="Close">×</button></div>
    ...
    <svg class="map terrain-map" id="world-map" viewBox="0 0 ${MAP_SIZE.join(' ')}" role="img" aria-label="Terrain map of Old Circle. North is up. Gold dots are kindled hearths; hollow rings are keepers; the white pointer is your position."><image href="/assets/map/world.svg" width="${MAP_SIZE[0]}" height="${MAP_SIZE[1]}"/>${names}${places}${fires}<g id="map-player" transform="translate(${x} ${y})"><circle r="3.7" fill="#fff8e5" stroke="#1c2c23" stroke-width="1"/><circle r="7" fill="none" stroke="#fff8e5" stroke-width=".7"/></g></svg>
    <p>Gold dots mark your kindled hearths. Filled keeper rings mark recovered seals.<br>Relief and contours show the slopes; pale lines follow the actual roads. Scale: the full map spans 480 × 640 metres.</p>`;
}
