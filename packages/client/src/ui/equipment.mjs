...
export function equipmentMarkup(player,rest){
  ...
  return `<div class="panel-top"><div><div class="eyebrow">Equipment & hearth forge</div><h2>What carries you onward</h2></div><button class="close" aria-label="Close">×</button></div>
    ...
    <div class="equipment-list">${Object.entries(CHARMS).map(([id,charm])=>{
      ...
      return `<article class="armor-row charm-row ${selected?'selected':''}"><div class="charm-description">${id!=='none'?`<img src="/assets/icons/charm-${id}.png" alt="" loading="lazy">`:''}<div><h4>${charm.name}</h4><p>${charm.description}${!owned?'<br>Found in '+charm.dungeon+'.':''}</p></div></div><button class="subtle" data-charm="${id}" ${rest.reason||!owned||selected?'disabled':''}>${selected?'Wearing':owned?(id==='none'?'Remove charm':'Wear'):'Not found'}</button></article>`;
