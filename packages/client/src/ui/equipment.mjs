...
export function equipmentMarkup(player,rest){
  ...
  return `<div class="panel-top"><div><div class="eyebrow">Equipment & hearth forge</div><h2>What carries you onward</h2></div><button class="close" aria-label="Close">×</button></div>
    ...
    <div class="equipment-list">${Object.entries(ARMOR).map(([id,armor])=>{
      ...
      return `<article class="armor-row ${selected?'selected':''}" id="equipment-armor-${id}" tabindex="-1" aria-labelledby="armor-title-${id}"><div><h4 id="armor-title-${id}">${armor.name}</h4><p>${armor.description}</p><dl><div><dt>Physical</dt><dd>${Math.round(armor.physical*100)}%</dd></div><div><dt>Magic</dt><dd>${Math.round(armor.magic*100)}%</dd></div><div><dt>Poise</dt><dd>${Math.round(armor.poise*100)}%</dd></div><div><dt>Pace</dt><dd>${Math.round(armor.speed*100)}%</dd></div><div><dt>Stamina / s</dt><dd>${armor.stamina}</dd></div><div><dt>Focus / s</dt><dd>${armor.focus}</dd></div><div><dt>Noise</dt><dd>${Math.round(armor.noise*100)}%</dd></div></dl></div><button class="subtle" data-armor="${id}" aria-label="${selected?'Wearing':'Wear'} ${armor.name}" aria-pressed="${selected}" ${rest.reason||!owned||selected?'disabled':''}>${selected?'Wearing':owned?'Wear':armor.seal+' seal'}</button></article>`;
    }).join('')}</div>
    <h3>Exploration charms</h3><p>Wear one charm at a time. Change it at a safe hearth. Each blessing carries a burden; the weapon figures above include your current charm.</p>
    <div class="equipment-list">${Object.entries(CHARMS).map(([id,charm])=>{
      ...
      return `<article class="armor-row charm-row ${selected?'selected':''}" id="equipment-charm-${id}" tabindex="-1" aria-labelledby="charm-title-${id}"><div class="charm-description">${id!=='none'?`<img src="/assets/icons/charm-${id}.png" alt="" loading="lazy">`:''}<div><h4 id="charm-title-${id}">${charm.name}</h4><p>${charm.description}${!owned?'<br>Found in '+charm.dungeon+'.':''}</p></div></div><button class="subtle" data-charm="${id}" aria-label="${selected?'Wearing':'Wear'} ${charm.name}" aria-pressed="${selected}" ${rest.reason||!owned||selected?'disabled':''}>${selected?'Wearing':owned?(id==='none'?'Remove charm':'Wear'):'Not found'}</button></article>`;
