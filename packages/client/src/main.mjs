...
const keys=new Set(),presses=new Map();let mouseDown=false;
...
<section class="hud" id="hud" hidden>
 ...
 <div class="compass"><div class="compass-diamond">◇</div><span id="heading">N</span><div class="compass-line"></div><span id="objective">ABBEY OF THE FIRST LIGHT</span></div>
...
</section><div id="modal-root"></div>`;

const $=s=>document.querySelector(s),send=data=>worker?.postMessage(data);
function modal(content){menu=true;keys.clear();presses.clear();mouseDown=false;document.exitPointerLock?.();send({type:'pause',paused:true});$('#modal-root').innerHTML=`<div class="modal"><section class="panel">${content}</section></div>`;$('#modal-root .close')?.addEventListener('click',closeModal);}
function closeModal(){menu=false;$('#modal-root').innerHTML='';send({type:'pause',paused:false});if(started)view?.engine.viewStack.el.focus();}
...
for(const b of document.querySelectorAll('[data-weapon]'))b.onclick=()=>send({type:'equip',weapon:b.dataset.weapon});$('#flask').onclick=()=>pulse('KeyR');
function pulse(code){keys.add(code);setTimeout(()=>keys.delete(code),100);}
window.addEventListener('keydown',e=>{
  if(['Space','Tab','KeyM'].includes(e.code))e.preventDefault();
  if(e.repeat)return;
  if(started&&['Tab','KeyM','Escape'].includes(e.code)){menu?closeModal():e.code==='KeyM'?map():journal();return;}
  keys.add(e.code);if(['Space','KeyQ','KeyR','KeyE'].includes(e.code))presses.set(e.code,performance.now()+100);if(e.code.startsWith('Digit')){const w=Object.keys(WEAPONS)[Number(e.code.slice(-1))-1];if(w)send({type:'equip',weapon:w});}
});
window.addEventListener('keyup',e=>keys.delete(e.code));window.addEventListener('blur',()=>{keys.clear();presses.clear();mouseDown=false;});
window.addEventListener('pointerdown',e=>{if(!started||menu||e.target.closest('button,select,input,.inspector'))return;if(e.button===0)mouseDown=true;if(!document.pointerLockElement)view.engine.viewStack.el.requestPointerLock?.();});
window.addEventListener('pointerup',e=>{if(e.button===0)mouseDown=false;});
window.addEventListener('pointermove',e=>{if(view&&!menu&&(document.pointerLockElement||(e.buttons&2))){view.yaw-=e.movementX*.003;view.pitch=Math.max(-.45,Math.min(.8,view.pitch+e.movementY*.002));}});
window.addEventListener('contextmenu',e=>e.preventDefault());
let previous=performance.now();
function frame(now){
  const dt=Math.min(.1,(now-previous)/1000);previous=now;
  if(!menu){
    const down=code=>keys.has(code)||(presses.get(code)??0)>now;
    const forward=(keys.has('KeyW')?1:0)-(keys.has('KeyS')?1:0),side=(keys.has('KeyD')?1:0)-(keys.has('KeyA')?1:0),yaw=view.yaw;
    const x=-Math.sin(yaw)*forward+Math.cos(yaw)*side,z=-Math.cos(yaw)*forward-Math.sin(yaw)*side;
...
function updateHud(){
  ...
  const heading=['N','NW','W','SW','S','SE','E','NE'][Math.round(((view.yaw%(Math.PI*2)+Math.PI*2)%(Math.PI*2))/(Math.PI/4))%8];$('#heading').textContent=heading;
