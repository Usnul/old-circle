...
import {handleDialogKeydown} from './ui/dialog-focus.mjs';
...
<section class="hud" id="hud" hidden>
 ...
 <div class="embers" id="embers">0</div><div class="hint" id="interact" hidden><kbd>E</kbd>Rest at the Pilgrim’s Hearth</div><div class="controls">WASD move · Shift sprint · C crouch · Space jump / mantle · Click / F attack · Q frost nova · R heal · Tab journal · M map</div>
...
updateSaveWarning();
function modal(content,{dismissible=true,shortcut,layout='default'}={}){
  if(!menu)modalReturnFocus=document.activeElement;
  menuCleanup?.();menuCleanup=null;
  menu=true;input?.suspend(true);send({type:'pause',paused:true});$('#capture-mouse').hidden=true;
  $('#modal-root dialog')?.close();
  $('#modal-root').innerHTML=`<dialog class="modal" aria-labelledby="modal-title"><section class="panel panel-${layout}"><div class="panel-content">${content}</div></section></dialog>`;
  ...
  dialog.addEventListener('keydown',e=>handleDialogKeydown(e,dialog,{shortcut:dismissible?shortcut:undefined,close:closeModal}));
  dialog.addEventListener('cancel',e=>{e.preventDefault();if(dismissible)closeModal();});
  const close=dialog.querySelector('.close');
  if(close){close.innerHTML=`Close <kbd>${shortcut==='Tab'?'Tab / Esc':shortcut==='m'?'M / Esc':'Esc'}</kbd>`;close.addEventListener('click',closeModal);}
  if(shortcut==='Tab')dialog.querySelector('.panel-top')?.insertAdjacentHTML('afterend','<p class="menu-navigation">↑ / ↓ select control · Enter activate · Shift + Tab previous control</p>');
...
function chooseOrigin(){modal(`<div class="panel-top"><h2>Choose character</h2><button class="close" aria-label="Close">×</button></div><p>Choose starting equipment and attributes.</p><div class="origins">${ORIGINS.map(o=>`<button class="origin ${o.id===origin?'selected':''}" data-origin="${o.id}" aria-pressed="${o.id===origin}"><img src="/assets/icons/${WEAPONS[o.weapon].icon}.png" alt=""><span class="origin-name">${o.name}</span><p>${WEAPONS[o.weapon].name}</p><div class="origin-stat">Vigor ${o.stats.vigor} · Endurance ${o.stats.endurance}<br>Might ${o.stats.might} · Insight ${o.stats.insight}</div></button>`).join('')}</div><div class="button-row"><button class="primary" id="enter"><span>Start game</span><span>⟶</span></button></div>`);for(const b of document.querySelectorAll('[data-origin]'))b.onclick=()=>{origin=b.dataset.origin;document.querySelectorAll('[data-origin]').forEach(x=>{x.classList.toggle('selected',x===b);x.setAttribute('aria-pressed',String(x===b));});};$('#enter').onclick=()=>{closeModal();start(null);};}
function controls(){modal(`<div class="panel-top"><h2>Controls</h2><button class="close" aria-label="Close">×</button></div><div class="help-grid">${[['Walk','W A S D'],['Look','Mouse / arrow keys'],['Sprint / release after exhaustion','Shift'],['Crouch / stealth','C'],['Jump / grab / climb','Space'],['Weapon attack','Left click / F'],['Frost nova','Q'],['Healing flask','R'],['Rest at hearth','E'],['Change equipment','1 – 4'],['Open / close journal','Tab'],['Open / close map','M'],['Journal / close menu','Escape'],['Select journal control','↑ / ↓'],['Activate control','Enter']].map(([a,b])=>`<div>${a}<kbd>${b}</kbd></div>`).join('')}</div><p>Click the world to look around. If the cursor cannot be captured, hold near a screen edge to keep turning. Arrow keys turn the camera and F attacks without mouse capture. Press the same menu shortcut again to close it. In the journal and equipment screens, use ↑ / ↓ to select controls and Enter to activate them; Shift + Tab selects the previous control. In other menus, Tab moves between controls. Escape closes any menu.</p><p>Attacks connect where the weapon meets the body. Watch enemy wind-ups, conserve stamina, and use the terrain. Release Space to hang at a ledge; Space climbs and C drops. The hearth restores your flasks and lets you improve your attributes. Other wanderers can join a boss battle already in progress.</p><p>Multiplayer requires the local server. If the connection goes away, your journey continues in the browser.</p>`);}
    ...
    worker.onmessage=({data})=>{
      ...
      if(data.type==='save'){
        if(inspecting){if(saveRequested)toast('Workshop journeys are temporary.');}
          ...
          if(saveRequested)toast(persisted?'Game saved.':journey.warning);
          else if(journey.warning!==previousWarning)toast(journey.warning||'Game saved.');
        }
      ...
      if(data.type==='level-result'){journalPending=false;toast(data.ok?'Attribute improved.':'A safe hearth and enough embers are required.');if(menu)refreshJournal();return;}
...
function refreshJournal(){
  ...
  const status=journalPending?'Improving attribute…':`Next improvement: ${cost} embers. ${reason??`Ready at ${rest.hearth.name}. Choose an attribute to improve.`}`;
...
function equipment(focusId){
  ...
  modal(equipmentMarkup(p,restStatus(p,snapshot.actors)),{shortcut:'Tab'});equipmentOpen=true;
  if(typeof focusId==='string')document.getElementById(focusId)?.focus({preventScroll:true});
  $('#modal-root .panel').scrollTop=scroll;
  const request=(type,item,button)=>{equipmentFocus=button.closest('article').id;toast('Updating equipment…');document.querySelectorAll('[data-armor],[data-reinforce],[data-charm]').forEach(b=>b.disabled=true);send({type,item});};
