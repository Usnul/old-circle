...
export class GameInput {
  constructor(engine,{action,look,captureChanged,error,inspect=false}){
    Object.assign(this,{engine,action,look,captureChanged,error,inspect});this.enabled=true;this.pending=new Map();this.axis=[0,0];this.capturePending=false;
  ...
  async start(){
    ...
    devices.pointer.on.move.add((_p,event,delta)=>{if(this.enabled&&(this.activeLook||(this.inspect&&(event.buttons&2))))this.look(delta.x,delta.y);});
    ...
    window.addEventListener('blur',()=>{this.pending.clear();this.suppressAttack=true;this.freeLook=false;this.element.classList.remove('free-look');this.captureChanged(false);});
  ...
  async capture(){
    ...
    try{await this.element.requestPointerLock();}
      ...
      this.error('Free mouse look is active. This browser cannot confine the cursor; Escape releases.');
    }
  }
  suspend(value){this.enabled=!value;this.pending.clear();if(value){this.freeLook=false;this.element.classList.remove('free-look');document.exitPointerLock();}}
