...
export default defineConfig({
  plugins:[nativeTextures,{name:'canonical-world-map',buildStart(){execFileSync(process.execPath,['tools/generate-world-map.mjs'],{cwd:fileURLToPath(new URL('../..',import.meta.url)),stdio:'inherit',windowsHide:true});}},compositionCapture(),{...strip({functions:['assert.*'],include:['**/*.js','**/*.mjs']}),apply:'build'}],
