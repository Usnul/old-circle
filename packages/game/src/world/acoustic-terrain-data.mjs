...
export class AcousticTerrainSurfaceSerializationAdapter extends BinaryClassSerializationAdapter {
  ...
  deserialize(buffer, mesh) {
    ...
    buffer.readFloat32Array(mesh.positions, 0, positions);
    const indices = buffer.readUint32();
    mesh.indices = new Uint32Array(indices);
    buffer.readUint32Array(mesh.indices, 0, indices);

    const bounds = new AABB3();
    deserializeAABB3(buffer, bounds);
    mesh.__bbox.set([...bounds]);
    mesh.__surface_area = buffer.readFloat64();

    const stride = buffer.readUint32();
    const nodes = buffer.readUint32();
    const root = buffer.readUint32();
    if (stride !== ELEMENT_WORD_COUNT) {
      throw new Error('Acoustic terrain needs rebuilding for this BVH layout');
    }
    const bytes = new Uint8Array(nodes * stride * 4);
    buffer.readBytes(bytes, 0, bytes.length);
    const tree = new BVH();
    tree.data_buffer = bytes.buffer;
    tree.allocate_linear(nodes);
    tree.root = root;
...
export function decodeAcousticTerrainSurface(bytes) {
  ...
  if (buffer.readUint32() !== WORLD_VERSION || buffer.readUTF8String() !== enginePackage.version) {
    throw new Error('Acoustic terrain needs rebuilding for this world and engine version');
  }
  return objectAdapter().deserialize(buffer);
}
