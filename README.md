...
An original dark fantasy action RPG built on **Meep 3.22.0**, with a shared persistent world and a reproducible Blender content pipeline. The game is under active development; the current playable world still needs substantial content and art work before release.
...
WASD walks, Shift sprints, C crouches, Space jumps and grabs nearby ledges. Keep Space held to climb; release it to hang, then press Space to climb or C to drop. Mouse or arrow keys look; left click or F attacks; Q casts frost nova; R drinks a flask; E rests at a nearby hearth. Keys 1–4 select owned weapons. After sprint exhaustion, recover at least 25 stamina (or a quarter of maximum) and release Shift before sprinting again. Tab opens the journal for attributes, equipment and the hearth forge, saving and the PvP toggle; M opens the map. Press the same shortcut again to close its menu; Tab also closes equipment. Escape releases the mouse and opens the journal, or closes an open menu. Menus fill the screen and keep keyboard focus inside. In the journal and equipment screens, Up/Down select controls, Enter activates them and Shift+Tab selects the previous control. Other menus use Tab and Shift+Tab to move between controls. Closing a menu returns keyboard control; click the world to resume mouse look. The journal pauses solo simulation; the shared world keeps running. In embedded browsers that reject pointer lock, free mouse look, edge turning and arrow keys remain available.
